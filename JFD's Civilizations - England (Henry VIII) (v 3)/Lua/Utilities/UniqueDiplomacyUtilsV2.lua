...
-- Passing nil as the targetResponseType will mean that this replaces all
-- entries that have the targetResponse
...
function ChangeDiplomacyResponse(leaderType, targetResponseType, targetResponse, newResponse, bias)
	if (leaderType and targetResponse and newResponse) then
		local sBiasSection = "";
		if (bias ~= nil) then sBiasSection = ", Bias = " .. bias end;
		...
		if text then
			if (targetResponseType == nil) then
				--print("DB.Query will be UPDATE Diplomacy_Responses SET Response = '" .. newResponse .. "'" .. sBiasSection .." WHERE LeaderType = '" ..leaderType.. "' AND Response= '" .. targetResponse .. "'");
				for _ in DB.Query("UPDATE Diplomacy_Responses SET Response = '" .. newResponse .. "'" .. sBiasSection .." WHERE LeaderType = '" ..leaderType.. "' AND Response= '" .. targetResponse .. "'") do end
				--print(leaderType .. "'s entries with Response " .. targetResponse .. " changed to use " .. newResponse .. "!");
			else
				--print("DB.Query will be UPDATE Diplomacy_Responses SET Response = '" .. newResponse .. "'" .. sBiasSection .." WHERE LeaderType = '" ..leaderType.. "' AND ResponseType = '" ..targetResponseType .. "' AND Response= '" .. targetResponse .. "'");
				for _ in DB.Query("UPDATE Diplomacy_Responses SET Response = '" .. newResponse .. "'" .. sBiasSection .." WHERE LeaderType = '" ..leaderType.. "' AND ResponseType = '" ..targetResponseType .. "' AND Response= '" .. targetResponse .. "'") do end
...
-- specified locale. If you do not supply a locale, it will try to replace in
-- the current one. This function assumes you will pass a valid locale if you 
-- do supply one (it expects it in the form "Language_??_??").
-- Please remember that you will lose the line you're replacing, so bear that
-- in mind if you need it later.
function ChangeDiplomacyGameTextToString(targetText, newString, locale)
	if (type(newString) == "string" and targetText) then
		if (locale == nil) then locale = "Language_" ..Locale.GetCurrentLanguage().Type end
...
-- locale. It will fail if the target entry doesn't exist in the
-- specified locale. If you do not supply a locale, it will try to replace in
-- the current one. This function assumes you will pass a valid locale if you 
-- do supply one (it expects it in the form "Language_??_??").
