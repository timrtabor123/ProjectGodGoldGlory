...
CivilopediaCategory[CategoryGovernments].buttonTexture = "civilopediatopbuttonsgovernment.dds";
...
CivilopediaCategory[CategoryGovernments].labelString = Locale.ConvertTextKey( "TXT_KEY_PEDIA_CATEGORY_17_LABEL" );
...
CivilopediaCategory[CategoryBuildings].PopulateList = function()
	...
	if(Game == nil or not Game.IsOption(GameOptionTypes.GAMEOPTION_NO_RELIGION)) then
		...
		local tableid = 1;
--		for building in DB.Query("SELECT Buildings.ID, Buildings.Description, Buildings.PortraitIndex, Buildings.IconAtlas, Buildings.IsWorldWonder, Buildings.IsNationalWonder from Buildings inner join  BuildingClasses on Buildings.BuildingClass = BuildingClasses.Type where Buildings.FaithCost > 0 and Buildings.Cost == -1 and Buildings.IsWorldWonder = 0 and Buildings.IsNationalWonder = 0 and BuildingClasses.MaxGlobalInstances < 0 and (BuildingClasses.MaxPlayerInstances <> 1 or Buildings.SpecialistCount > 0) and BuildingClasses.MaxTeamInstances < 0;") do
...
--		SELECT Buildings.ID, Buildings.Description, Buildings.PortraitIndex, Buildings.IconAtlas
	local sql = [[
		...
		WHERE (FaithCost == 0 or Buildings.Cost >= 0) AND IsNationalWonder = 0 AND BuildingClasses.MaxGlobalInstances < 0 AND (BuildingClasses.MaxPlayerInstances < 1 or Buildings.SpecialistCount > 0) AND BuildingClasses.MaxTeamInstances < 0 AND Technologies.Era = ?;]];
...
--				SELECT Buildings.ID, Buildings.Description, Buildings.PortraitIndex, Buildings.IconAtlas
			local sql = [[
				...
				FROM Buildings
				INNER JOIN BuildingClasses ON Buildings.BuildingClass = BuildingClasses.Type
				WHERE Buildings.PrereqTech IS NULL AND (Buildings.FaithCost == 0 or Buildings.Cost >= 0)  AND IsNationalWonder = 0 AND BuildingClasses.MaxGlobalInstances < 0 AND (BuildingClasses.MaxPlayerInstances < 1 or Buildings.SpecialistCount > 0) AND BuildingClasses.MaxTeamInstances < 0;]];
...
--	for building in GameInfo.Buildings() do
	for building in ShowInPediaIterator("Buildings") do
		local thisBuildingClass = GameInfo.BuildingClasses[building.BuildingClass];
		if building.IsNationalWonder then
			local article = {};
			local name = Locale.ConvertTextKey( building.Description )
			article.entryName = name;
			article.entryID = building.ID;
...
-- JFD
-----------------------------------------------------------------------
CivilopediaCategory[CategoryGovernments].PopulateList = function()
	print("CivilopediaCategory[CategoryGovernments].PopulateList")
	-- add the instances of the policy entries

	sortedList[CategoryGovernments] = {};

	--Governments
	local branchID = 0;

	sortedList[CategoryGovernments][branchID] = {};
	local tableid = 1;

 	for government in ShowInPediaIterator("JFD_Governments") do
		local article = {};
		local name = Locale.ConvertTextKey( government.Description )
		article.entryName = name;
		article.entryID = government.ID;
		article.entryCategory = CategoryGovernments;
		article.tooltipTextureOffset, article.tooltipTexture = IconLookup( government.PortraitIndex, buttonSize, government.IconAtlas );
		if not article.tooltipTextureOffset then
			article.tooltipTexture = defaultErrorTextureSheet;
			article.tooltipTextureOffset = nullOffset;
		end

		sortedList[CategoryGovernments][branchID][tableid] = article;
		tableid = tableid + 1;

		-- index by various keys
		searchableList[Locale.ToLower(name)] = article;
		searchableTextKeyList[government.Description] = article;
		categorizedList[(CategoryGovernments * absurdlyLargeNumTopicsInCategory) + government.ID] = article;
	end

	-- sort this list alphabetically by localized name
	table.sort(sortedList[CategoryGovernments][branchID], Alphabetically);
	
	-- for each reform branch
--	for branch in GameInfo.JFD_Reforms() do
	for branch in ShowInPediaIterator("JFD_ReformBranches") do
		...
		-- for each reform in this branch
-- 		for reform in GameInfo.JFD_Reforms("ReformBranchType = '" .. branch.Type .. "'") do
 		for reform in ShowInPediaIterator("JFD_Reforms", "ReformBranchType = '" .. branch.Type .. "'") do
			local article = {};
			local name = Locale.ConvertTextKey( reform.ReformSubBranchDescription) .. " (" .. Locale.ConvertTextKey( reform.ShortDescription ) .. ")"
			article.entryName = name;
			article.entryID = reform.ID;
			article.entryCategory = CategoryGovernments;
			-- article.tooltipTextureOffset, article.tooltipTexture = IconLookup( reform.PortraitIndex, buttonSize, reform.IconAtlas );
			-- if not article.tooltipTextureOffset then
				-- article.tooltipTexture = defaultErrorTextureSheet;
				-- article.tooltipTextureOffset = nullOffset;
			-- end
			...
			-- index by various keys
			searchableList[Locale.ToLower(name)] = article;
			searchableTextKeyList[reform.Description] = article;
			categorizedList[(CategoryGovernments * absurdlyLargeNumTopicsInCategory) + reform.ID] = article;
		end
...
--		for religion in GameInfo.Religions("Type <> 'RELIGION_PANTHEON'") do
	...
	if g_IsRealReligionsActive then
		sectionConditions = {
			"Pantheon = 1",
			"Founder = 1",
			"Follower = 1 AND Worshipper = 0",
			"Worshipper = 1",
			"Enhancer = 1",
			"Reformation = 1"
		};
...
-- JFD
--------------------------------------------------------------------
CivilopediaCategory[CategoryGovernments].DisplayHomePage = function()
	ClearArticle();
	Controls.ArticleID:SetText( Locale.ConvertTextKey( "TXT_KEY_PEDIA_JFD_GOVERNMENTS_PAGE_LABEL" ));

	local portraitIndex = 0;
	local portraitAtlas = "JFD_SOVEREIGNTY_NOTIFICATION_ATLAS";

	for row in DB.Query("SELECT PortraitIndex, IconAtlas from JFD_Governments Where IconAtlas IS NOT NULL ORDER By Random() LIMIT 1") do
		portraitIndex = row.PortraitIndex;
		portraitAtlas = row.IconAtlas;
	end
	...
	UpdateTextBlock( Locale.ConvertTextKey( "TXT_KEY_PEDIA_QUOTE_BLOCK_JFD_GOVERNMENTS" ), Controls.HomePageBlurbLabel, Controls.HomePageBlurbInnerFrame, Controls.HomePageBlurbFrame );
	...
	local thisBBTextInstance = g_BBTextManager:GetInstance();
	if thisBBTextInstance then
		thisBBTextInstance.BBTextHeader:SetText( Locale.ConvertTextKey( "TXT_KEY_PEDIA_JFD_GOVERNMENTS_PAGE_LABEL" ));
		UpdateSuperWideTextBlock( Locale.ConvertTextKey( "TXT_KEY_PEDIA_JFD_GOVERNMENTS_HELP_TEXT" ), thisBBTextInstance.BBTextLabel, thisBBTextInstance.BBTextInnerFrame, thisBBTextInstance.BBTextFrame );
	end
	Controls.BBTextStack:SetHide( false );
	ResizeEtc();
end;
...
CivilopediaCategory[CategoryHomePage].SelectArticle = function( pageID, shouldAddToList )
	...
	elseif pageID == CategoryWorldCongress then
		...
		CivilopediaCategory[pageID].DisplayHomePage();
	elseif pageID == 17 then
		if selectedCategory ~= CategoryGovernments then
		SetSelectedCategory(CategoryGovernments);
		end
		CivilopediaCategory[CategoryGovernments].DisplayHomePage() ;
	else
...
-- Putmalk: If this tech grants Research Agreements, and Research Agreements are enabled, then display it on the tech info page
		if g_IsCivIVDiplomacyActive then
...
CivilopediaCategory[CategoryUnits].SelectArticle = function( unitID, shouldAddToList )
	...
	if unitID ~= -1 then
		...
		if thisUnit.CombatClass then
			if g_IsCPActive and thisUnit.IsMounted then
...
function SelectBuildingOrWonderArticle( buildingID )
	if buildingID ~= -1 then
		...
		-- update the district
		-- if(Game ~= nil) then
			-- local districtID = Players[Game.GetActivePlayer()]:GetBuildingDistrict(thisBuilding.Type)
			-- if districtID > -1 then
				-- Controls.DistrictFrame:SetHide( false );
				-- local district = GameInfo.JFD_Districts[districtID]
				-- local districtDesc = district.Description
				-- Controls.DistrictLabel:LocalizeAndSetText(districtDesc);
			-- end
		-- end
...
--							if (GameInfo.Buildings[tReward.Building] ~= nil and GameInfo.Buildings[tReward.Building].ID == buildingID) then
		...
		if perTurnCost > 0 then
			Controls.MaintenanceLabel:SetText( tostring(perTurnCost).." [ICON_MAINTENANCE]" );
		...
		-- JFD
		-- JFD update the Development
		-- local developmentItems = {};
		-- local iDevelopment = GetBuildingYieldModifier(buildingID, "YIELD_JFD_DEVELOPMENT");
		-- if(iDevelopment > 0) then
			-- table.insert(developmentItems, "+" .. tostring(iDevelopment).."% [ICON_JFD_DEVELOPMENT]" );
		-- elseif(iDevelopment < 0) then
			-- table.insert(developmentItems, tostring(iDevelopment).."% [ICON_JFD_DEVELOPMENT]" );
		-- end

		-- JFD
		-- update the Development Change
		-- local iDevelopment = GetBuildingYieldChange(buildingID, "YIELD_JFD_DEVELOPMENT");
		-- if(iDevelopment > 0) then
			-- table.insert(developmentItems, "+" .. tostring(iDevelopment).." [ICON_JFD_DEVELOPMENT]" );
		-- elseif(iDevelopment < 0) then
			-- table.insert(developmentItems, tostring(iDevelopment).." [ICON_JFD_DEVELOPMENT]" );
		...
		-- if(#developmentItems > 0) then
			-- Controls.DevelopmentLabel:SetText( table.concat(developmentItems, ", ") );
			-- Controls.DevelopmentFrame:SetHide( false );
		-- end
		...
		-- update the Loyalty
		-- local loyaltyItems = {};
		-- local iLoyalty = GetBuildingYieldChange(buildingID, "YIELD_JFD_LOYALTY");
		-- if(iLoyalty > 0) then
			-- table.insert(loyaltyItems, "+" .. tostring(iLoyalty).." [ICON_PRODUCTION]");
		-- end
		...
		-- if(iLoyalty ~= 0) then
			...
			-- table.insert(loyaltyItems, "" .. tostring(iLoyalty).."% [ICON_JFD_LOYALTY]" );
		-- end
		-- if(#loyaltyItems > 0) then
			-- Controls.LoyaltyLabel:SetText( table.concat(loyaltyItems, ", ") );
			-- Controls.LoyaltyFrame:SetHide( false );
...
--				local thisBuildingInfo = GameInfo.Buildings[buildingClass.DefaultBuilding];
				...
				if (thisBuildingInfo) then
					...
					if thisBuildingInstance then
						...
						if thisBuildingInfo.IsWorldWonder == 1 or thisBuildingInfo.IsNationalWonder == 1 then
...
-- JFD
---------------------------------------------------------------------------------------
CivilopediaCategory[CategoryGovernments].SelectArticle = function( governmentID, shouldAddToList )	
	print("CivilopediaCategory[CategoryGovernments].SelectArticle");
	if selectedCategory ~= CategoryGovernments then
		SetSelectedCategory(CategoryGovernments);
	end
	...
	if shouldAddToList == addToList then
		currentTopic = currentTopic + 1;
		listOfTopicsViewed[currentTopic] = categorizedList[(CategoryGovernments * absurdlyLargeNumTopicsInCategory) + governmentID];
		for i = currentTopic + 1, endTopic, 1 do
			listOfTopicsViewed[i] = nil;
		end
		endTopic = currentTopic;
	...
	if governmentID ~= -1 then
		...
		local thisGovernment = GameInfo.JFD_Governments[governmentID];

		-- update the name
		local name = Locale.ConvertTextKey( thisGovernment.Description );
		Controls.ArticleID:SetText( name );

		-- update the portrait
		if IconHookup( thisGovernment.PortraitIndex, portraitSize, thisGovernment.IconAtlas, Controls.Portrait ) then
			Controls.PortraitFrame:SetHide( false );
		else
			Controls.PortraitFrame:SetHide( true );
		end
		...
		-- update the game info
		if thisGovernment.Help then
			UpdateTextBlock( Locale.ConvertTextKey( thisGovernment.Help ), Controls.GameInfoLabel, Controls.GameInfoInnerFrame, Controls.GameInfoFrame );
		end

		-- update the strategy info
		--UpdateTextBlock( Locale.ConvertTextKey( thisGovernment.Strategy ), Controls.StrategyLabel, Controls.StrategyInnerFrame, Controls.StrategyFrame );

		-- update the historical info
		if (thisGovernment.Civilopedia) then
			UpdateTextBlock( Locale.ConvertTextKey( thisGovernment.Civilopedia ), Controls.HistoryLabel, Controls.HistoryInnerFrame, Controls.HistoryFrame );
		end
...
CivilopediaCategory[CategoryGovernments].SelectArticle2 = function( reformID, shouldAddToList )
	print("CivilopediaCategory[CategoryGovernments].SelectArticle");
	if selectedCategory ~= CategoryGovernments then
		SetSelectedCategory(CategoryGovernments);
	end
	...
	if shouldAddToList == addToList then
		currentTopic = currentTopic + 1;
		listOfTopicsViewed[currentTopic] = categorizedList[(CategoryGovernments * absurdlyLargeNumTopicsInCategory) + reformID];
		for i = currentTopic + 1, endTopic, 1 do
			listOfTopicsViewed[i] = nil;
		end
		endTopic = currentTopic;
	...
	if reformID ~= -1 then
		...
		local thisReform = GameInfo.JFD_Reforms[reformID];

		-- update the name
		local name = Locale.ConvertTextKey( thisReform.ReformSubBranchDescription) .. " (" .. Locale.ConvertTextKey( thisReform.ShortDescription ) .. ")"
		Controls.ArticleID:SetText( name );

		-- update the portrait
		if IconHookup( 14, portraitSize, "JFD_SOVEREIGNTY_NOTIFICATION_ATLAS", Controls.Portrait ) then
			Controls.PortraitFrame:SetHide( false );
		else
			Controls.PortraitFrame:SetHide( true );
		end
		...
		-- update the game info
		if thisReform.HelpBonus then
			local strHelp = Locale.ConvertTextKey("{1_Desc}[NEWLINE]----------[NEWLINE][COLOR_POSITIVE_TEXT]Bonuses:[ENDCOLOR][NEWLINE]----------[NEWLINE][ICON_BULLET]{2_Desc}[NEWLINE]----------[NEWLINE][COLOR_NEGATIVE_TEXT]Penalties:[ENDCOLOR][NEWLINE]----------[NEWLINE][ICON_BULLET]{3_Desc}[NEWLINE]----------", thisReform.Pedia, thisReform.HelpBonus, thisReform.HelpPenalty)
			UpdateTextBlock( strHelp, Controls.GameInfoLabel, Controls.GameInfoInnerFrame, Controls.GameInfoFrame );
		end
		
		local branch = thisReform.ReformBranchType
		local branchName = Locale.ConvertTextKey( GameInfo.JFD_ReformBranches[branch].ShortDescription );
		Controls.ReformBranchLabel:SetText( branchName );
		Controls.ReformBranchFrame:SetHide( false );
		
		local branchName = Locale.ConvertTextKey( thisReform.ReformSubBranchDescription );
		Controls.ReformSubBranchLabel:SetText( branchName );
		Controls.ReformSubBranchFrame:SetHide( false );
				
		if thisReform.PrereqEra then
			local era = ShowInPediaLookup("Eras", thisReform.PrereqEra);
			if era then
				local eraName = Locale.ConvertTextKey( era.Description );
				Controls.PrereqEraLabel:SetText( eraName );
				Controls.PrereqEraFrame:SetHide( false );
		...
		-- update the strategy info
		--UpdateTextBlock( Locale.ConvertTextKey( thisReform.Strategy ), Controls.StrategyLabel, Controls.StrategyInnerFrame, Controls.StrategyFrame );

		-- update the historical info
		-- if (thisReform.Civilopedia) then
			-- UpdateTextBlock( Locale.ConvertTextKey( thisReform.Civilopedia ), Controls.HistoryLabel, Controls.HistoryInnerFrame, Controls.HistoryFrame );
		-- end
...
CivilopediaCategory[CategoryCityStates].SelectArticle = function( cityStateID, shouldAddToList )
	...
	if cityStateID ~= -1 then
		...
		if (thisCityState.Civilopedia) then
			if Game.IsSuzeraintyActive() then
				UpdateTextBlock( "[COLOR_POSITIVE_TEXT]Suzerain Bonus:[ENDCOLOR][NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_SUZERAINTRAIT_" .. thisCityState.Type), Controls.GameInfoLabel, Controls.GameInfoInnerFrame, Controls.GameInfoFrame );
			end
...
-- JFD
-------------------------------------------------------
CivilopediaCategory[CategoryGovernments].SelectHeading = function( selectedBranchID, dummy )
	print("CivilopediaCategory[CategoryGovernments].SelectHeading");
	g_ListHeadingManager:ResetInstances();
	g_ListItemManager:ResetInstances();

	sortedList[CategoryGovernments][selectedBranchID].headingOpen = not sortedList[CategoryGovernments][selectedBranchID].headingOpen; -- ain't lua great
	...
	if thisListInstance then
		sortOrder = sortOrder + 1;
		thisListInstance.ListItemLabel:SetText( Locale.ConvertTextKey( "TXT_KEY_PEDIA_CATEGORY_17_LABEL" ));
		thisListInstance.ListItemButton:SetVoids( homePageOfCategoryID, addToList );
		thisListInstance.ListItemButton:RegisterCallback( Mouse.eLClick, CivilopediaCategory[CategoryGovernments].buttonClicked );
		thisListInstance.ListItemButton:SetToolTipCallback( TipHandler );
		otherSortedList[tostring( thisListInstance.ListItemButton )] = sortOrder;
	end

	local governmentsID = 0
	-- add a branch header
	local thisHeadingInstance = g_ListHeadingManager:GetInstance();
	if thisHeadingInstance then
		sortOrder = sortOrder + 1;
		if sortedList[CategoryGovernments][governmentsID].headingOpen then
			local textString = "TXT_KEY_PEDIA_CATEGORY_17_LABEL";
			local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( textString );
			thisHeadingInstance.ListHeadingLabel:SetText( localizedLabel );
		else
			local textString = "TXT_KEY_PEDIA_CATEGORY_17_LABEL";
		...
		thisHeadingInstance.ListHeadingButton:SetVoids( governmentsID, 0 );
		thisHeadingInstance.ListHeadingButton:RegisterCallback( Mouse.eLClick, CivilopediaCategory[CategoryGovernments].SelectHeading );
		otherSortedList[tostring( thisHeadingInstance.ListHeadingButton )] = sortOrder;
	end

	-- for each element of the sorted list
	if sortedList[CategoryGovernments][governmentsID].headingOpen then
		for i, v in ipairs(sortedList[CategoryGovernments][governmentsID]) do
			local thisListInstance = g_ListItemManager:GetInstance();
			if thisListInstance then
				sortOrder = sortOrder + 1;
				thisListInstance.ListItemLabel:SetText( v.entryName );
				thisListInstance.ListItemButton:SetVoids( v.entryID, addToList );
				thisListInstance.ListItemButton:RegisterCallback( Mouse.eLClick, CivilopediaCategory[CategoryGovernments].SelectArticle );
				thisListInstance.ListItemButton:SetToolTipCallback( TipHandler )
				otherSortedList[tostring( thisListInstance.ListItemButton )] = sortOrder;
			end
		end
	...
	--	for branch in GameInfo.JFD_ReformBranches() do
	for branch in ShowInPediaIterator("JFD_ReformBranches") do
		...
		if thisHeadingInstance then
			sortOrder = sortOrder + 1;
			if sortedList[CategoryGovernments][branchID].headingOpen then
				local textString = branch.Description;
				local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( textString );
				thisHeadingInstance.ListHeadingLabel:SetText( localizedLabel );
			else
		...
		-- for each element of the sorted list
		if sortedList[CategoryGovernments][branchID].headingOpen then
			for i, v in ipairs(sortedList[CategoryGovernments][branchID]) do
				local thisListInstance = g_ListItemManager:GetInstance();
				if thisListInstance then
					sortOrder = sortOrder + 1;
					thisListInstance.ListItemLabel:SetText( v.entryName );
...
CivilopediaCategory[CategoryBeliefs].SelectHeading = function( selectedSectionID, dummy )
	...
	local numSections = 6
	if g_IsRealReligionsActive then numSections = 7 end
...
CivilopediaCategory[CategoryPolicies].DisplayList = function()
	...
	for branch in GameInfo.PolicyBranchTypes("PurchaseByLevel = 0") do
	-- for branch in ShowInPediaIterator("PolicyBranchTypes") do
	...
	for branch in GameInfo.PolicyBranchTypes("PurchaseByLevel = 1") do
	-- for branch in ShowInPediaIterator("PolicyBranchTypes, PurchaseByLevel = '0'") do
...
-- JFD
--------------------------------------------------------------------------------
CivilopediaCategory[CategoryGovernments].DisplayList = function()
	g_ListHeadingManager:ResetInstances();
	g_ListItemManager:ResetInstances();
	...
	if thisListInstance then
		sortOrder = sortOrder + 1;
		thisListInstance.ListItemLabel:SetText( Locale.ConvertTextKey( "TXT_KEY_PEDIA_JFD_GOVERNMENTS_PAGE_LABEL" ));
		thisListInstance.ListItemButton:SetVoids( homePageOfCategoryID, addToList );
		thisListInstance.ListItemButton:RegisterCallback( Mouse.eLClick, CivilopediaCategory[CategoryGovernments].buttonClicked );
		thisListInstance.ListItemButton:SetToolTipCallback( TipHandler );
		otherSortedList[tostring( thisListInstance.ListItemButton )] = sortOrder;
	...
	local thisHeadingInstance = g_ListHeadingManager:GetInstance();
	if thisHeadingInstance then
		sortedList[CategoryGovernments][governmentsID].headingOpen = true; -- ain't lua great
		sortOrder = sortOrder + 1;
		local textString = "TXT_KEY_PEDIA_CATEGORY_17_LABEL";
		local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( textString );
		print(textString)
...
--	for branch in GameInfo.JFD_ReformBranches() do
	for branch in ShowInPediaIterator("JFD_ReformBranches") do
		...
		local thisHeadingInstance = g_ListHeadingManager:GetInstance();
		if thisHeadingInstance then
			sortedList[CategoryGovernments][branchID].headingOpen = true; -- ain't lua great
			sortOrder = sortOrder + 1;
			local textString = branch.Description;
			local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( textString );
			thisHeadingInstance.ListHeadingLabel:SetText( localizedLabel );
		...
		for i, v in ipairs(sortedList[CategoryGovernments][branchID]) do
			...
			if thisListInstance then
				...
				thisListInstance.ListItemLabel:SetText( v.entryName );
				thisListInstance.ListItemButton:SetVoids( v.entryID, addToList );
				thisListInstance.ListItemButton:RegisterCallback( Mouse.eLClick, CivilopediaCategory[CategoryGovernments].SelectArticle2 );
				thisListInstance.ListItemButton:SetToolTipCallback( TipHandler )
				otherSortedList[tostring( thisListInstance.ListItemButton )] = sortOrder;
			end
		end
...
function ClearArticle()
	...
	Controls.PortraitFrame:SetHide( true );
	Controls.DistrictFrame:SetHide( true );
	Controls.CostFrame:SetHide( true );
	...
	Controls.DevelopmentFrame:SetHide( true );
	...
	Controls.ReformBranchFrame:SetHide( true );
	Controls.ReformSubBranchFrame:SetHide( true );
	Controls.TenetLevelFrame:SetHide(true);
...
function ShowHideHandler( isHide )
    if( isHide ) then
		...
		LuaEvents.CivilopediaScreenClosed()
	else
