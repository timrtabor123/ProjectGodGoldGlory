...
------------------------------------------------------------------------------------------------------------------------
--local userSettingCIDColoniesCore 			= (g_GetUserSetting("JFD_CID_COLONIES_CORE") == 1)
--local userSettingCIDColoniesDistanceRadius	= (g_GetUserSetting("JFD_CID_COLONIES_DISTANCE_CORE_CITY_RADIUS"))
------------------------------------------------------------------------------------------------------------------------
--local promotionColonistID					= GameInfoTypes["PROMOTION_JFD_COLONIAL_SETTLER"]
...
local GetActionIconIndexAndAtlas = {
	...
	[ActionSubTypes.ACTIONSUBTYPE_MISSION] = function(action, unit)
...
function HookupActionIcon(action, actionIconSize, icon, unit)
	if action.SubType == ActionSubTypes.ACTIONSUBTYPE_MISSION and (action.Type == "MISSION_CREATE_GREAT_WORK" and unit:GetUnitClassType() == unitClassProphetID) then
		IconHookup(1, actionIconSize, "JFD_GREAT_RELICS_ICON_ATLAS", icon);	
	else
    ...
    -- do
        ...
        then
            ...
            if(instance.UnitActionIcon ~= nil) then
				HookupActionIcon(action, actionIconSize, instance.UnitActionIcon, unit);	
            end
...
function UpdateUnitStats(unit)
	...
	--JFD WORKFORCES
	if Player.GetNumUnitBuildCharges then
		local control = Controls
		local numCharges, numChargesMax = activePlayer:GetNumUnitBuildCharges(nil, unit, unit:GetUnitType()) 
...
function TipHandler( control )
	...
	elseif (action.Type == "MISSION_TRADE") then
		...
		if unitClass == "UNITCLASS_MERCHANT" then
			...
			strToolTip = strToolTip .. "[NEWLINE]" .. Locale.Lookup("TXT_KEY_MISSION_CONDUCT_TRADE_MISSION_HELP_JFD_DIGNITARY");
		end
		if (not bDisabled) then
			strToolTip = strToolTip .. "[NEWLINE]----------------[NEWLINE]";
	...
	------------------------------------------------------------------------------------------------------------------------
	-- Found Colony
	--elseif (action.Type == "MISSION_FOUND" and unit:HasPromotion(promotionColonistID)) then
	--	strToolTip = strToolTip .. "[NEWLINE]" .. Locale.Lookup("TXT_KEY_MISSION_BUILD_JFD_COLONY_HELP");
	-- Join Council
	...
	if (bBuild) then
		...
		--JFD WORKFORCES
		if Player.GetUnitBuildChargeCost then 
		...
		do
			...
			if (iYieldChange ~= 0) then
				...
				elseif (iYield == GameInfoTypes["YIELD_JFD_DEVELOPMENT"]) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_DEVELOPMENT_STRING", iYieldChange);
				elseif (iYield == GameInfoTypes["YIELD_JFD_LOYALTY"]) then
					strBuildYieldString = strBuildYieldString .. Locale.ConvertTextKey("TXT_KEY_BUILD_LOYALTY_STRING", iYieldChange);
				end
				-------------------------------------------------
				--JFD ENDS
				-------------------------------------------------
		...
		--local improvementDefense = activePlayer:GetImprovementDefense(improvementID)
		--if improvementDefense > 0 then
		--	strBuildYieldString = strBuildYieldString .. "[NEWLINE]" .. Locale.ConvertTextKey("TXT_KEY_BUILD_YIELD_STRING", improvementDefense, "[ICON_STRENGTH]", "TXT_KEY_YIELD_JFD_DEFENSE");
		--end
		-------------------------------------------------
		--JFD ENDS
		-------------------------------------------------
	...
	-------------------------------------------------
	--if (action.Type == "MISSION_FOUND") then
	--	if unit:HasPromotion(promotionColonistID) then
	--		strTitleString = Locale.ConvertTextKey("TXT_KEY_MISSION_BUILD_JFD_COLONY")
	--	end	
	--end
	-------------------------------------------------
	-- CLAIMS
	 -------------------------------------------------
	--JFD Dignitary hack
	--if action.Type == "MISSION_TRADE" then
	--	if unit:IsUnitClass(unitClassDignitaryID) then
	--		strTitleString =  "[COLOR_POSITIVE_TEXT]" .. Locale.ConvertTextKey("TXT_KEY_MISSION_CONDUCT_JFD_TRADE_MISSION_DIGNITARY_DESC") .. "[ENDCOLOR]"
	--	end
	--end
    -------------------------------------------------
	-- JFD END
