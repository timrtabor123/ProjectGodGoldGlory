...
local function JFD_CyclesOfPower_CityConstructed(playerID, cityID, buildingID, isGold, isFaith)
	...
	if numVirtue > 0 then
		...
		if playerIsHuman then
			local city = player:GetCityByID(cityID)
			Events.GameplayAlertMessage(g_ConvertTextKey("TXT_KEY_ALERT_MESSAGE_JFD_VIRTUE_BUILDING", city:GetName()))
		end
	end
end
...
local function JFD_CyclesOfPower_PlayerAdoptPolicy(playerID, policyID)
	...
	if numVirtue > 0 then
		...
		local playerIsHuman = (player:IsHuman() and player:IsTurnActive())
		if playerIsHuman then
			Events.GameplayAlertMessage(g_ConvertTextKey("TXT_KEY_ALERT_MESSAGE_JFD_VIRTUE_POLICY"))
		end
	end
end
...
local function JFD_CyclesOfPower_TeamTechResearched(teamID, techID)
	...
	if numVirtue > 0 then
		...
		local playerIsHuman = (player:IsHuman() and player:IsTurnActive())
		if playerIsHuman then
			Events.GameplayAlertMessage(g_ConvertTextKey("TXT_KEY_ALERT_MESSAGE_JFD_VIRTUE_TECH"))
		end
	end
end
