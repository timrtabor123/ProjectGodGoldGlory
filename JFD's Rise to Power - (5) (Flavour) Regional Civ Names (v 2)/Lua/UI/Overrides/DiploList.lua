...
function UpdateDisplay()
	...
	if(PreGame.GetCivilizationShortDescription(g_iPlayer) ~= "") then
		...
		TruncateString(Controls.CivName, textBoxSize, Locale.ConvertTextKey(g_pPlayer:GetCivilizationShortDescription()));
	end
	...
	for iPlayerLoop = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
		...
		if (iPlayerLoop ~= g_iPlayer and pOtherPlayer:IsAlive()) then
			...
			if (g_pTeam:IsHasMet(iOtherTeam)) then
				...
				local strCiv = pOtherPlayer:GetCivilizationShortDescription();
