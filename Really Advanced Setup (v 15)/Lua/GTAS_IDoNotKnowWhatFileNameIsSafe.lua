...
-------------------------------------------------------------------------------------------------------------------------------------------------
-- GTAS_IDoNotKnowWhatFileNameIsSafe - Part of Really Advanced Setup Mod
--
-- A big thank you to whoward69 for writing this code.
--
-- GTAS_PlotAreaSpiralIterator has been changed by GeneralTso.
-- The passed value "centre" has been replaced by "rMin". More comments available near the function itself.
-------------------------------------------------------------------------------------------------------------------------------------------------
...
--   GTAS_PlotAreaSpiralIterator(pPlot, r, sector, anticlock, inwards, centre) returns each plot within an area of radius r in concentric rings
...
--    end
--
--    for pAreaPlot in GTAS_PlotAreaSpiralIterator(pPlot, r, sector, anticlock, inwards, centre) do
...
-- Passed variable "centre" (boolean) removed and passed variable "rMin" added by GeneralTso.
-- Passed variable "r" name was change to "rMax" for clarity.
-- This new value is the minimum radius. Only plots from rMin to rMax are now returned.
-- rMin = 0 works the same as centre = true, while rMin = 1 works the same as centre = false.
function GTAS_PlotAreaSpiralIterator(pPlot, rMin, rMax, sector, anticlock, inwards)
  -- print(string.format("GTAS_PlotAreaSpiralIterator((%i, %i), r=%i, s=%i, d=%s, w=%s, rMin=%i)", pPlot:GetX(), pPlot:GetY(), r, (sector or SECTOR_NORTH), (anticlock and "rev" or "fwd"), (inwards and "in" or "out"), rMin))
  -- This coroutine walks each ring in sequence
  local next = coroutine.create(function ()
    if (rMin == 0 and not inwards) then
      coroutine.yield(pPlot)
    end

    if (inwards) then
      for i=rMax, rMin, -1 do
...
function TestGTAS_PlotAreaSpiralIterator(pPlot, r, sector, anticlock, inwards, centre, highlight)
  for pAreaPlot in GTAS_PlotAreaSpiralIterator(pPlot, r, sector, anticlock, inwards, centre) do
...
-- TestGTAS_PlotAreaSpiralIterator(Players[0]:GetCapitalCity():Plot(), 3, SECTOR_SOUTH, DIRECTION_ANTICLOCKWISE, DIRECTION_OUTWARDS, CENTRE_INCLUDE)
