...
------------------------------------------------------------------------------
-- Modified for Really Advanced Startup Mod.
function CivNeedsCoastalStart(civType)
	-- This function retrieves Start Along Ocean bias from the database.
	for row in DB.Query("SELECT * FROM Civilization_Start_Along_Ocean where CivilizationType = ?", civType) do
		if row.StartAlongOcean == true then
			-- print(civType, "CivNeedsCoastalStart:  return true!!!");
			return true;
		end
	end

	-- print(civType, "CivNeedsCoastalStart:  return false");	
	return false;
end
...
function CivNeedsRiverStart(civType)
	-- This function retrieves Start Along River bias from the database.
	for row in DB.Query("SELECT * FROM Civilization_Start_Along_River where CivilizationType = ?", civType) do
		if row.StartAlongRiver == true then
			-- print(civType, "CivNeedsRiverStart:  return true!!!");
			return true;
		end
	end
	
	-- print(civType, "CivNeedsRiverStart: return false");	
	return false;
end
...
function CivNeedsPlaceFirstCoastalStart(civType)
	for row in DB.Query("SELECT * FROM Civilization_Start_Place_First_Along_Ocean where CivilizationType = ?", civType) do
		if row.PlaceFirst == true then
			-- print(civType, "CivNeedsPlaceFirstCoastalStart:  return true!!!");
			return true;
		end
	end

	-- print(civType, "CivNeedsPlaceFirstCoastalStart:  return false");	
	return false;
end
...
function GetNumStartRegionPriorityForCiv(civType)
	...
	for row in DB.Query("select count(*) as count from Civilization_Start_Region_Priority where CivilizationType = ?", civType) do
		-- print(civType, "GetNumStartRegionPriorityForCiv: return", row.count);	
		return row.count;
	end
	
	-- print(civType, "GetNumStartRegionPriorityForCiv: return", 0);	
	return 0;
end
...
function GetNumStartRegionAvoidForCiv(civType)
	...
	for row in DB.Query("select count(*) as count from Civilization_Start_Region_Avoid where CivilizationType = ?", civType) do
		-- print(civType, "GetNumStartRegionAvoidForCiv: return", row.count);		
		return row.count;
	end
	
	-- print(civType, "GetNumStartRegionAvoidForCiv: return", 0);		
	return 0;
end
...
function GetStartRegionPriorityListForCiv_GetIDs(civType)
	-- print(civType, "GetStartRegionPriorityListForCiv_GetIDs");
	...
	for row in DB.Query("SELECT * FROM Civilization_Start_Region_Priority where CivilizationType = ?", civType) do
...
function GetStartRegionAvoidListForCiv_GetIDs(civType)
	-- print(civType, "GetStartRegionAvoidListForCiv_GetIDs");
	-- This function returns a sorted list of all Start Region Avoid for this civ.
	-- List will include ID of each region type, plus be sorted and contain only unique values.
	
	local avoidRegionTypes = {};
	
	for row in DB.Query("SELECT * FROM Civilization_Start_Region_Avoid where CivilizationType = ?", civType) do
...
function GetStartRegionPriorityListForCiv_GetTypes(civType)
	-- print(civType, "GetStartRegionPriorityListForCiv_GetTypes");
...
function GetStartRegionAvoidListForCiv_GetTypes(civType)
	-- print(civType, "GetStartRegionAvoidListForCiv_GetTypes");
