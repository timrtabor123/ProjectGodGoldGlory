print("This is the 'UI - Religion Spread' replacment for religionOverview.lua")
...
function GetBeliefType(belief)
	...
	elseif(isBNW and belief.Reformation) then
	    return Locale.Lookup("TXT_KEY_RO_BELIEF_TYPE_REFORMATION");
	else
		return Locale.Lookup("TXT_KEY_RO_BELIEF_TYPE_OTHER");
	end
end
...
Controls.GreatPersonName7:SetHide(true)
Controls.GreatPersonName8:SetHide(true)
Controls.GreatPersonName9:SetHide(true)
	...
	AddGreatPersonUnit("UNIT_PROPHET", function()
		return (player:HasCreatedReligion() or (player:HasCreatedPantheon() and Game.GetNumReligionsStillToFound() > 0));
	end);
	...
	CivIconHookup(player:GetID(), 80, Controls.FounderIcon, Controls.FounderIconBG, Controls.FounderIconShadow, (isUsingFlagsPlus == false), (isUsingFlagsPlus == false) );
	...
	elseif (player:HasCreatedPantheon()) then
	  ...
	  local pantheonBeliefs = player.GetBeliefsInPantheon and player:GetBeliefsInPantheon() or {player:GetBeliefInPantheon()}
	  for _,iBelief in ipairs(pantheonBeliefs) do
...
function RefreshWorldReligions()
	...
	if(#religions > 0) then
      ...
      entry.ReligionName:SetColor(v.Color, 0);
			...
			CivIconHookup(v.FounderID, 45, entry.FounderIcon, entry.FounderIconBG, entry.FounderIconShadow, (isUsingFlagsPlus == false), (isUsingFlagsPlus == false) );
	...
	for iPlayer = 0, GameDefines.MAX_CIV_PLAYERS - 1 do	
		...
		if (pPlayer:IsEverAlive()) then
			...
			elseif (pPlayer:HasCreatedPantheon()) then
			  local pantheonBeliefs = pPlayer.GetBeliefsInPantheon and pPlayer:GetBeliefsInPantheon() or {pPlayer:GetBeliefInPantheon()}
			  for _,iBelief in ipairs(pantheonBeliefs) do
				local belief = GameInfo.Beliefs[iBelief];
