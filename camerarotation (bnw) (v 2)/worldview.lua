...
local db = Modding.OpenUserData("test", 0);
print("Hello World");
print(file);
...
function CityCaptured(player, capital, iX, iY, newPlayer, conquest, conquest)
...
function( wParam, lParam )
	...
	if ( wParam == Keys.VK_LEFT ) and bShift and rotatingDirection ~= 1 then
		if rotatingDirection == -1 then
			Events.CameraStopRotatingCW()
		end
		Events.CameraStartRotatingCCW()
		rotatingDirection = 1
	elseif ( wParam == Keys.VK_RIGHT ) and bShift and rotatingDirection ~= -1 then
		if rotatingDirection == 1 then
		...
		rotatingDirection = -1
	elseif ( wParam == Keys.VK_LEFT ) and not bShift then
		-- stop rotating
		if rotatingDirection == 1 then
			Events.CameraStopRotatingCCW()
			rotatingDirection = 0
		...
		return true;
	elseif ( wParam == Keys.VK_RIGHT ) and not bShift then
		-- stop rotating
		if rotatingDirection == -1 then
			Events.CameraStopRotatingCW()
			rotatingDirection = 0
	...
	elseif (wParam == Keys.VK_DIVIDE) then
		...
		return true;
        elseif (wParam >= 48 and wParam <= 57) then
		if (typeHop == xType) then
			xPosHop = 10*xPosHop + wParam - 48;
		else
			yPosHop = 10*yPosHop + wParam - 48;
		end
                return true;
	elseif (( wParam == Keys.VK_SPACE ) and bShift) then
		if (typeHop == xType) then
			typeHop = yType;
		else
			local plot = Map.GetPlot(xPosHop, yPosHop);
			if (plot ~= nil) then
	                	UI.LookAt(plot, 0);
				typeHop = xType;
